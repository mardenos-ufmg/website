...
ui = function(ns) {
  ...
  fluidRow(style = "height: 500px;",
    ...
    column(4, plotOutput(ns("fa-scores-summary-plot")))
  ),
  fluidRow(column(12, DT::dataTableOutput(ns("fa-scores-summary")))),
    
  section("Loadings (EEE e SU)",
    fluidRow(
      column(7, plotOutput(ns("fa-loadings-eee"))),
      column(5, plotOutput(ns("fa-loadings-su")))
    ),
    collapse = T
  )
